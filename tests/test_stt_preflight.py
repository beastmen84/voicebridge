...
def test_stt_preflight_treats_alignment_models_as_optional(monkeypatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(
        stt_preflight,
        "inspect_stt_runtime",
        lambda _python_path, timeout_seconds=stt_preflight.STT_RUNTIME_INSPECTION_TIMEOUT_SECONDS: {
            "torch_ok": True,
            "torch_version": "test",
            "cuda_build": "",
...
def test_stt_preflight_retries_transient_torch_inspection_failure(monkeypatch, tmp_path: Path) -> None:
    python_path = tmp_path / ".venv-ml" / "Scripts" / "python.exe"
    worker_path = tmp_path / "stt_worker.py"
    model_dir = tmp_path / "models" / "whisperx"
    site_packages = tmp_path / ".venv-ml" / "Lib" / "site-packages"
    ...
    def fake_inspect(_python_path, timeout_seconds=stt_preflight.STT_RUNTIME_INSPECTION_TIMEOUT_SECONDS):
        attempts.append(timeout_seconds)
        return {
            "torch_ok": len(attempts) > 1,
            "torch_version": "test" if len(attempts) > 1 else "",
            "cuda_build": "",
            "cuda_available": False,
            "cuda_device_count": 0,
            "cuda_device_name": "",
    ...
    monkeypatch.setattr(stt_preflight, "inspect_stt_runtime", fake_inspect)

    ok, _summary, details, runtime_info = stt_preflight.check_stt_preflight()

    assert ok
    assert attempts == [
        stt_preflight.STT_RUNTIME_INSPECTION_TIMEOUT_SECONDS,
        stt_preflight.STT_RUNTIME_INSPECTION_RETRY_TIMEOUT_SECONDS,
    ]
    assert runtime_info["torch_ok"]
    assert any("passed after one retry" in line for line in details)
