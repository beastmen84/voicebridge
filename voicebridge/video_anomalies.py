    ...
    *,
    ...
    try:
        ...
        while True:
            ...
            if current is not None and previous is not None and previous_current_diff is not None:
                diff_next = _frame_distance(current, prepared, cv2=cv2)
                diff_skip = _frame_distance(previous, prepared, cv2=cv2)
                time_seconds = (current_frame_number / fps) if fps > 0 else 0.0
                anomaly = classify_frame_anomaly(
                    frame_number=current_frame_number,
                    time_seconds=time_seconds,
                    diff_prev=previous_current_diff,
                    diff_next=diff_next,
                    diff_skip=diff_skip,
                ...
                if anomaly is not None and current_frame_number - last_added_frame >= min_gap_frames:
                    ...
                    if len(anomalies) >= max_results:
                        break
                previous_current_diff = diff_next
            elif current is not None:
                previous_current_diff = _frame_distance(current, prepared, cv2=cv2)
            ...
            if on_progress is not None and total_frames > 0:
                progress = int(min(99, max(0, (current_frame_number / total_frames) * 100)))
